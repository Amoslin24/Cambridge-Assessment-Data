    ...
    <>
      <div className="mt-8 rounded-2xl border border-slate-200 p-5">
        <h2 className="text-lg font-bold text-slate-900">
          {tr('班级宏观分析（分层与进步汇总）', 'Class Macro Analysis (Stratification & Progress)')}
        </h2>
        <p className="mt-1 text-sm text-slate-600">
          {tr(
            '基于当前筛选项（班级、组别、级别、日期范围等）聚合全班样本：左侧为能力分层，右侧为在筛选范围内具备纵向对比条件学生的换算分差分布。',
            'Aggregate class samples under current filters (class, set, level, date range, etc.): stratification on the left and converted-score delta distribution of longitudinally comparable students on the right.',
          )}
        </p>

        {classMacro.mode === 'EMPTY' && (
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
            {tr('当前无可聚合记录。', 'No records available for macro aggregation.')}
          </div>
        )}
          ...
          <>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">{tr('学生人数（去重）', 'Students (deduplicated)')}</div>
                <div className="text-xl font-bold text-slate-900">{classMacro.studentCount}</div>
                <div className="mt-1 text-xs text-slate-500">
                  {tr('考试记录条数：', 'Exam records: ')}
                  {classMacro.examRecordCount}
                </div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">
                  {tr('最近一次换算总分 · 均值', 'Latest converted total - Mean')}
                </div>
                <div className="text-xl font-bold text-slate-900">
                  {classMacro.latestTotalMean !== null
                    ? classMacro.mode === 'YLE'
                      ? `${classMacro.latestTotalMean} ${tr('盾', 'shields')}`
                      : `${classMacro.latestTotalMean} ${tr('分', 'pts')}`
                    : '—'}
                </div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">
                  {tr('最近一次换算总分 · 中位数', 'Latest converted total - Median')}
                </div>
                <div className="text-xl font-bold text-slate-900">
                  {classMacro.latestTotalMedian !== null
                    ? classMacro.mode === 'YLE'
                      ? `${classMacro.latestTotalMedian} ${tr('盾', 'shields')}`
                      : `${classMacro.latestTotalMedian} ${tr('分', 'pts')}`
                    : '—'}
                </div>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-xs text-slate-500">{tr('纵向可比学生数', 'Longitudinally comparable students')}</div>
                <div className="text-xl font-bold text-slate-900">
                  {classMacro.progress?.eligibleStudents ?? 0}
                </div>
                <div className="mt-1 text-xs text-slate-500">
                  {tr('单次考试仅一条：', 'Single-exam students: ')}
                  {classMacro.progress?.singleExamStudents ?? 0}
                  {tr(' 人', '')}
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
              <div>
                <div className="text-sm font-semibold text-slate-900">
                  {tr('能力分层（最近一次）', 'Ability Stratification (Latest)')}
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  {tr(
                    'YLE 按总盾段划分；MSE 按量表总分与常见达标线分段，便于教研对齐干预优先级。',
                    'YLE is stratified by total shields; MSE is segmented by scale score ranges and common benchmark lines for intervention prioritization.',
                  )}
                </p>
                <div className="mt-3 h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={classMacro.stratification} layout="vertical" margin={{ left: 8, right: 16 }}>
                      ...
                      />
                      <Legend />
                      <Bar dataKey="count" name={tr('学生人数', 'Students')} fill="#0d9488" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
              <div>
                <div className="text-sm font-semibold text-slate-900">
                  {tr('进步分差分布（筛选范围内）', 'Progress Delta Distribution (Within Filters)')}
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  {classMacro.mode === 'YLE'
                    ? tr(
                        '横轴为总盾分差（末次减首次，整数）；仅统计同一 YLE 口径下至少两条记录的学生。',
                        'X-axis shows total shield delta (latest-first, integer); only students with at least two records under the same YLE scale are included.',
                      )
                    : tr(
                        '横轴为量表总分分差（末次减首次）；仅统计同一 MSE 口径下至少两条记录的学生。',
                        'X-axis shows total scale-score delta (latest-first); only students with at least two records under the same MSE scale are included.',
                      )}
                </p>
                {classMacro.progress && classMacro.progress.eligibleStudents > 0 ? (
                  <div className="mt-3 h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={classMacro.progress.deltaHistogram}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="count" name={tr('学生人数', 'Students')} fill="#4f46e5" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                ) : (
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                    {tr(
                      ...
                      'No students have at least two exams under the same conversion scale within current filters, so delta distribution is not rendered.',
                    )}
                  </div>
                )}
              </div>
      ...
      <div className="mt-8 rounded-2xl border border-slate-200 p-5">
        <h2 className="text-lg font-bold text-slate-900">
          {tr('班级/级别分布图（换算总分）', 'Class/Level Distribution (Converted Total)')}
        </h2>
        <p className="mt-1 text-sm text-slate-600">
          {tr(
            '按考试级别统计：在各级别内对每位学生取其「该级别下最近一次考试」的换算总分并做分布；选择单一级别时与顶部「级别」筛选一致。',
            'By exam level: within each level, use each student’s latest converted total to build distributions; for a single level, this follows the top level filter.',
          )}
        </p>
        ...
        {selectedLevel === 'ALL' ? (
          <>
            <p className="mt-2 text-xs text-slate-500">
              {tr(
                ...
                'Currently set to "All levels"; charts below are split by level (student counts are independent per level).',
              )}
            </p>
            {distributionByLevel.length === 0 ? (
              <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                {tr('当前筛选下无可用样本。', 'No samples available under current filters.')}
              </div>
            ) : (
                      ...
                      <div className="mt-3 h-72">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={row.distribution.data}>
                            ...
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="count" name={tr('学生人数', 'Students')} fill="#2563eb" />
                          </BarChart>
                        </ResponsiveContainer>
                      </div>
                    )}
          ...
          <>
            <div className="mt-2 text-xs text-slate-500">
              {tr('当前级别：', 'Current level: ')}
              {selectedLevel}
              {tr('；统计样本 ', '; sample size ')}
              {distributionSourceRecords.length}
              {tr(' 人。', ' students. ')}
      ...
      <div className="mt-8 rounded-2xl border border-slate-200 p-5">
        <h2 className="text-lg font-bold text-slate-900">
          {tr('班级题段均分（最近一次）', 'Class Part Mean (Latest)')}
        </h2>
        <p className="mt-1 text-sm text-slate-600">
          {tr(
            '在当前筛选与级别口径下，对每位学生取该级别「最近一次」试卷，将各题段原始分在全班求平均并按该题段满分折算为正确率（%）。柱形越低表示该题段班级层面相对越薄弱，橙色虚线对应 60% 正确率基准。',
            'Within current filters and level scope, use each student’s latest paper and compute class mean raw scores per part, then convert to accuracy (%). Lower bars indicate relatively weaker class performance, and the orange dashed line marks the 60% benchmark.',
          )}
        </p>
        {classPartMeansByContext.every((section) => section.blocks.length === 0) ? (
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
            {tr(
              ...
              'No part-level aggregate data is available under current conditions (e.g., no records in this level).',
            )}
          </div>
        ) : (
            ...
            >
              <div className="flex flex-wrap items-baseline gap-2">
                ...
                <span className="text-xs text-slate-500">
                  ...
                  {tr(' 人，每人取该级别最近一次考试）', ' students, each uses latest exam in this level)')}
                </span>
              </div>
              <div className="mt-3 grid gap-6 lg:grid-cols-2">
                {section.blocks.map((block) => (
                  ...
                  >
                        ...
                        <BarChart data={block.data}>
                          ...
                          <Tooltip
                            formatter={(value, _name, item) => {
                              ...
                              return [
                                tr(
                                  `${pct ?? value}%（班均原始 ${raw ?? '—'}/${partMax ?? '—'}）`,
                                  `${pct ?? value}% (class mean raw ${raw ?? '—'}/${partMax ?? '—'})`,
                                ),
                                tr('班均正确率', 'Class mean accuracy'),
                              ];
                            }}
                          />
                          <Legend />
                          <Bar
                            dataKey="meanPercent"
                            name={tr('班均正确率（%）', 'Class mean accuracy (%)')}
                            fill="#0369a1"
                            radius={[4, 4, 0, 0]}
                          />
                        </BarChart>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {tr('橙色虚线：60% 正确率基准。', 'Orange dashed line: 60% accuracy benchmark.')}
                    </p>
                  </div>
                ))}
              </div>
